...
app.get('/api/clientes', (req, res) => {
  const query = 'SELECT * FROM clientes'; // Consulta para obtener todos los clientes
  ...
  db.query(query, (err, results) => {
    ...
    res.status(200).json(results);  // Devolver los resultados (clientes) en formato JSON
  });
});
...
app.post('/api/clientes', (req, res) => {
  ...
  if (!nombres || !apellido_paterno || !dni || !celular) {
    return res.status(400).json({ message: "Los campos 'Nombres', 'Apellido Paterno', 'DNI' y 'Celular' son obligatorios." });
  }

  // Inserción en la base de datos
  const query = `
    INSERT INTO clientes (nombres, apellido_paterno, apellido_materno, dni, direccion, celular, correo)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `;

  db.query(query, [nombres, apellido_paterno, apellido_materno, dni, direccion, celular, correo], (err, result) => {
    if (err) {
      console.error('Error al insertar el cliente:', err);
      return res.status(500).json({ message: 'Error al insertar el cliente.' });
    }

    // Devuelve el cliente insertado, incluyendo el id generado por MySQL
    const clienteCreado = {
      id: result.insertId,
      nombres,
      apellido_paterno,
...
app.post('/api/clientes/expediente/:dni', (req, res) => {
  ...
  const expedientePath = path.resolve('expedientes', dni);
...
app.get('/api/aportes', (req, res) => {
  const query = 'SELECT * FROM aportes';
  db.query(query, (err, results) => {
    if (err) {
      return res.status(500).json({ message: 'Error al obtener los aportes' });
    }
    res.json(results);
  });
});
...
// Ruta para agregar un nuevo aporte
app.post('/api/aportes', (req, res) => {
  const { id_cliente, id_usuario, monto, fecha_aporte, descripcion } = req.body;

  const query = 'INSERT INTO aportes (id_cliente, id_usuario, monto, fecha_aporte, descripcion) VALUES (?, ?, ?, ?, ?)';
  db.query(query, [id_cliente, id_usuario, monto, fecha_aporte, descripcion], (err, result) => {
    if (err) {
      return res.status(500).json({ message: 'Error al agregar el aporte' });
    }
    res.status(201).json({ id: result.insertId, id_cliente, id_usuario, monto, fecha_aporte, descripcion });
  });
});


// Ruta para eliminar un aporte
app.delete('/api/aportes/:id', (req, res) => {
  ...
  db.query(query, [id], (err, result) => {
    if (err) {
      return res.status(500).json({ message: 'Error al eliminar el aporte' });
    }
...
// Ruta para obtener todos los clientes
app.get('/api/clientes', (req, res) => {
  const query = 'SELECT id, nombres, apellido_paterno, apellido_materno FROM clientes';
  db.query(query, (err, results) => {
    if (err) {
      return res.status(500).json({ message: 'Error al obtener los clientes' });
    }
    res.json(results); // Regresa la lista de clientes
  });
});

// Ruta para obtener los usuarios
app.get('/api/usuarios', (req, res) => {
  const query = 'SELECT id, nombres, apellido_paterno, apellido_materno FROM usuarios';
  
  // Ejecutar la consulta para obtener los usuarios de la base de datos
  db.query(query, (err, results) => {
    if (err) {
      // Si ocurre un error en la consulta, responder con un error 500
      return res.status(500).json({ message: 'Error al obtener los usuarios' });
    }

    // Responder con la lista de usuarios obtenida de la base de datos
    res.json(results);
  });
});
